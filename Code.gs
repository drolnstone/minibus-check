...
/* ==========================================================================
   ...
   It used to be the phone's. The passenger page made up a random handle,
   kept it in that browser's storage, and the handle was the whole of the
   identity: one booking per browser, and a family who booked on the laptop
   and then opened the page on a phone was a stranger to the record. They
   booked again. Two rows, two lots of seats, one family, and cancelling on
   one device left the other device's seats standing on the driver's list
   where nobody could see them to take them off.

   So the identity is the phone NUMBER now, which is the thing that actually
   belongs to the person rather than to the glass in their hand. Give it once
   on any device and that device is holding your booking, wherever it was
   made.

   Two columns, doing two different jobs:

     Phone         the number itself, in plain sight, because the coordinator
                   ringing round on a Sunday morning needs to be able to.
     Passenger ID  a one-way fingerprint of that number. It is what the page
                   sends on every poll from then on, so the number itself
                   travels once, in one POST, and never again sits in a web
                   address or in a log line.

   The fingerprint cannot be turned back into a number. It exists so the
   lookups are cheap and quiet, not to hide anything from the person who owns
   this spreadsheet: the number is in the next column along.
   ========================================================================== */
...
/**
 * Adds any column this version writes that an older sheet does not have yet.
 * Same shape as ensureChecksColumns and ensureTripColumns: only ever appends
 * on the right, so every row already on the tab keeps its meaning.
 */
function ensureBookingColumns(sh) {
  if (structFresh("bookings")) return;

  if (sh.getMaxColumns() < BOOKINGS_HEADERS.length) {
    sh.insertColumnsAfter(sh.getMaxColumns(),
                          BOOKINGS_HEADERS.length - sh.getMaxColumns());
  }
  var lastCol = sh.getLastColumn();
  var head = lastCol
    ...
    : [];
  BOOKINGS_HEADERS.forEach(function (name, i) {
    if (head.indexOf(name) !== -1) return;
  ...
  try { sh.getRange(2, 9, Math.max(1, sh.getMaxRows() - 1), 1).setNumberFormat("@"); }
...
function readBookings(ss, key) {
  ...
  if (!sh || sh.getLastRow() < 2) return [];
  /* Never wider than the tab actually is. A sheet still on the old eight
     columns is read eight wide and the two new fields come back empty, which
     is exactly what an old row means. Asking for ten on an eight column tab
     would throw, and it would throw inside the call that loads the page for
     the whole congregation. */
  var width = Math.min(BOOKINGS_HEADERS.length, sh.getMaxColumns());
 ...
 *
 * It used to be one booking per device, which is how a family who booked on
 * the laptop and then opened the page on a phone ended up holding two.
 */
...
function handleBookingLocked(b) {
  ...
  /* A new booking has to say whose it is. The whole point of the number is
     that one person holds one booking however many devices they own, and a
     row with nobody's name on it is the old fault walking back in through
     the door it was shown out of.
  ...
  if (existing) {
    ...
    if (ref) sh.getRange(existing.row, 7).setValue(ref);
    ...
    if (pid && !existing.pid) {
      sh.getRange(existing.row, 9, 1, 2).setValues([[phone ? "'" + phone : "", pid]]);
    }
  } else {
