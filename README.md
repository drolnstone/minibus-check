...
12. **Alerts, separately.** A rehearsal sends no push at all, deliberately, and
    ...
    on, then use **Send me a test** on the hub, or **Send me a test alert** on
    the passenger panel. A notification should arrive within a few seconds
...
get five, and no more:

- **the bus is not running** — sent the moment the Rota says that route is off,
  which is the only one of the five that can arrive before the morning
...
driver's hub where the Turn on offer was; **Send me a test alert** sits on the
passenger panel. It pushes to that one phone and to nobody else.
